 ...
 * @version 	$Id: RevisionController.php 2316 2010-04-17 12:33:20Z huuphuoc $
 * @since		2.0.4
 */
...
class News_RevisionController extends Zend_Controller_Action 
{
	...
	public function addAction() 
	{
		...
		if ($request->isPost()) {
			...
			$revision = new Tomato_Modules_News_Model_Revision(array(
	...
	public function listAction()
	{
		...
		$revisions = $revisionDao->find($start, $perPage, $exp);
	...
	public function restoreAction()
	{
		...
		if ($request->isPost()) {
			...
			$revision = $revisionDao->getRevisionById($id);
			if (null == $revision) {
				$this->getResponse()->setBody($response);
				return;
			}
			...
			// Update article
			$user = Zend_Auth::getInstance()->getIdentity();
			$articleDao->update(new Tomato_Modules_News_Model_Article(array(
