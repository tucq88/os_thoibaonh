...
class Tomato_Modules_News_Widgets_Rating_Widget extends Tomato_Core_Widget 
{
	protected function _prepareShow() 
	{
		...
		if ($articleId) {
			...
			$rateDao = Tomato_Core_Model_Dao_Factory::getInstance()->setModule('news')->getRateDao();
			$rateDao->setDbConnection($conn);
			$total = $rateDao->countRating($articleId);
	...
	protected function _prepareRating() 
	{
		...
		if ($articleId > 0 && $rate > 0 && $rate <= $numStar) {
			...
			$rateDao = Tomato_Core_Model_Dao_Factory::getInstance()->setModule('news')->getRateDao();
			$rateDao->setDbConnection($conn);
			$rateDao->addRating(new Tomato_Modules_News_Model_Rate(array(
