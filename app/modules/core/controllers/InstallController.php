 ...
 * @version 	$Id: InstallController.php 2328 2010-04-17 14:03:48Z huuphuoc $
 * @since		2.0.1
 */
...
class Core_InstallController extends Zend_Controller_Action 
{
	 ...
	 * @since 2.0.4
	 */
	private static $_SAMPLE_DATA = array(
		'mysql' 	=> '/install/tomatocms_sample_db.sql',
		'pdo_mysql' => '/install/tomatocms_sample_db.sql',
	);
	...
	public function step2Action() 
	{
		...
		if ('saveConfig' == $mode || $request->isPost()) {
			...
			foreach (array('host', 'port', 'username', 'password', 'dbname') as $key) {
				$master[$key] = $request->getPost($key);
			}
			
			$datetimeFormat = $request->getPost('datetimeFormat');			
			...
			$config['web']['lang'] 				= $request->getPost('lang');
			$config['web']['lang_direction'] 	= $request->getPost('langDirection');
			$config['web']['meta_keyword'] 		= preg_replace("/\s+/", ' ', strip_tags($request->getPost('metaKeyword')));
			$config['web']['meta_description'] 	= $request->getPost('metaDescription');
			$config['web']['default_title'] 	= $request->getPost('title');
			 ...
			 * @since 2.0.3
			 */
			$offline = $request->getPost('offline');
			$offline = ($offline) ? 1 : 0;
			if ($offline){
			...
			$config['web']['offline_message'] 		= $request->getPost('offlineMessage');
			$config['datetime']['timezone'] 		= $request->getPost('timezone');
			...
			if ($mode != 'install') {
				...
				try {
					...
					$templateDao 	= Tomato_Core_Model_Dao_Factory::getInstance()->setModule('core')->getTemplateDao();
					 ...
					 */
					$privilegeInstaller = new Tomato_Modules_Core_Services_Install_Privilege();
					...
					if ($importSampleData) {
						...
						// TODO: Get database adapter from form
						$adapter 	= 'Pdo_Mysql';
						$adapter 	= strtolower($adapter);
						$file 		= isset(self::$_SAMPLE_DATA[$adapter]) ? self::$_SAMPLE_DATA[$adapter] : null;
						$importer 	= Tomato_Modules_Core_Import_ImporterFactory::getInstance();
