 ...
 * @version 	$Id: PluginController.php 2334 2010-04-17 14:50:51Z huuphuoc $
 */
...
class Core_PluginController extends Zend_Controller_Action 
{
	...
	public function installAction() 
	{
		...
		if ($request->isPost()) {
			...
			if ($info) {
				...
				$pluginDao 	= Tomato_Core_Model_Dao_Factory::getInstance()->setModule('core')->getPluginDao();
				$pluginDao->setDbConnection($conn);
				$id = $pluginDao->add(new Tomato_Modules_Core_Model_Plugin($info));
	...
	public function listAction() 
	{
		...
		$pluginDao = Tomato_Core_Model_Dao_Factory::getInstance()->setModule('core')->getPluginDao();
		$pluginDao->setDbConnection($conn);
		$rs = $pluginDao->getOrderedPlugins();
		if ($rs) {
			foreach ($rs as $row) {
